...
int main(int argc, char const *argv[]) {
  ...
  parms.set_coeff_modulus(CoeffModulus::Create(poly_modulus_degree, { 50, 30, 30, 30, 30, 50 }));
  ...
  cout << "Evaluating polynomial -0.0007634 * x^8 -0.00001257 * x^7 + 0.01584 * x^6 + 0.00001584 * x^5 - 0.1188 * x^4 - 0.0005484 * x^3 + 0.4935 * x^2 + 0.5004 * x + 0.1009 ..." << endl;
  double coeff8 = -0.0007634;
  double coeff7 = -0.00001257;
  double coeff6 = 0.01584;
  double coeff5 = 0.0001584;
  double coeff4 = -0.1188;
  double coeff3 = -0.00005483;
  double coeff2 = 0.4935;
  double coeff1 = 0.5004;
  double coeff0 = 0.1009;
  vector<double> v_coeff8, v_coeff7, v_coeff6, v_coeff5, v_coeff4, v_coeff3, v_coeff2, v_coeff1, v_coeff0;
  Plaintext plain_coeff8, plain_coeff7, plain_coeff6, plain_coeff5, plain_coeff4, plain_coeff3, plain_coeff2, plain_coeff1, plain_coeff0;
  Ciphertext ct_coeff8, ct_coeff7, ct_coeff6, ct_coeff5, ct_coeff4, ct_coeff3, ct_coeff2, ct_coeff1, ct_coeff0;

  cout << "Encode vectors." << endl;
  for(int64_t i=0 ; i<slot_count ; i++){
    v_coeff8.push_back(coeff8);
    v_coeff7.push_back(coeff7);
  ...
  encoder.encode(v_coeff8, scale, plain_coeff8);
  encryptor.encrypt(plain_coeff8, ct_coeff8);
  encoder.encode(v_coeff7, scale, plain_coeff7);
  encryptor.encrypt(plain_coeff7, ct_coeff7);
  ...
  evaluator.multiply_inplace(x5_encrypted, ct_coeff5);
  ...
  Ciphertext x7_encrypted = x_encrypted;
  evaluator.multiply_inplace(x7_encrypted, ct_coeff7);
  evaluator.relinearize_inplace(x7_encrypted, relin_keys);
  cout << "    + Scale of x7_encrypted before rescale: " << log2(x7_encrypted.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(x7_encrypted);
  cout << "    + Scale of x7_encrypted after rescale: " << log2(x7_encrypted.scale()) << " bits" << endl;
  ...
  cout << "    + Modulus chain index for x7_encrypted: "
       << context.get_context_data(x7_encrypted.parms_id())->chain_index() << endl;
  cout << "    + Modulus chain index for x4_encrypted: "
       << context.get_context_data(x4_encrypted.parms_id())->chain_index() << endl;
  evaluator.mod_switch_to_next_inplace(x7_encrypted);
  evaluator.multiply_inplace(x7_encrypted, x4_encrypted);
  evaluator.relinearize_inplace(x7_encrypted, relin_keys);
  cout << "    + Scale of x^4 before rescale: " << log2(x7_encrypted.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(x7_encrypted);
  cout << "    + Scale of x^4 after rescale: " << log2(x7_encrypted.scale()) << " bits" << endl;
  Ciphertext x7_encrypted_coeff7 = x2_encrypted;
  cout << "    + Modulus chain index for x7_encrypted_coeff7: "
       << context.get_context_data(x7_encrypted_coeff7.parms_id())->chain_index() << endl;
  cout << "    + Modulus chain index for x7_encrypted: "
       << context.get_context_data(x7_encrypted.parms_id())->chain_index() << endl;
  evaluator.mod_switch_to_next_inplace(x7_encrypted_coeff7);
  evaluator.mod_switch_to_next_inplace(x7_encrypted_coeff7);
  evaluator.multiply_inplace(x7_encrypted_coeff7, x7_encrypted);

  cout<<"Compute x^8."<<endl;
  Ciphertext x8_encrypted;
  evaluator.square(x4_encrypted, x8_encrypted);
  evaluator.relinearize_inplace(x8_encrypted, relin_keys);
  cout << "    + Scale of x8_encrypted before rescale: " << log2(x8_encrypted.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(x8_encrypted);
  cout << "    + Scale of x8_encrypted after rescale: " << log2(x8_encrypted.scale()) << " bits" << endl;

  cout << "Parameters used by all four terms are different." << endl;
  cout << "    + Modulus chain index for x8_encrypted: "
       << context.get_context_data(x8_encrypted.parms_id())->chain_index() << endl; //
  cout << "    + Modulus chain index for ct_coeff8: "
       << context.get_context_data(ct_coeff8.parms_id())->chain_index() << endl; //
  cout<<"Compute coeff8 * x^8."<<endl;
  Ciphertext x8_encrypted_coeff8 = x8_encrypted;
  evaluator.mod_switch_to_next_inplace(ct_coeff8);
  evaluator.mod_switch_to_next_inplace(ct_coeff8);
  evaluator.mod_switch_to_next_inplace(ct_coeff8);
  evaluator.multiply_inplace(x8_encrypted_coeff8, ct_coeff8);
  ...
  cout<<"Compute coeff6 * x^2 * x^4."<<endl;
  Ciphertext x6_encrypted_coeff6 = x4_encrypted;
  evaluator.multiply_inplace(x6_encrypted_coeff6, x6_encrypted);

  cout << "Parameters used by all four terms are different." << endl;
  cout << "    + Modulus chain index for x8_encrypted_coeff8: "
       << context.get_context_data(x8_encrypted_coeff8.parms_id())->chain_index() << endl; //
  cout << "    + Modulus chain index for x7_encrypted_coeff7: "
       << context.get_context_data(x7_encrypted_coeff7.parms_id())->chain_index() << endl; //
  ...
  cout << "    + Exact scale in x8_encrypted_coeff8: " << x8_encrypted_coeff8.scale() << endl;
  cout << "    + Exact scale in x7_encrypted_coeff7: " << x7_encrypted_coeff7.scale() << endl;
  cout << "    + Exact scale in x6_encrypted_coeff6 : " << x6_encrypted_coeff6.scale() << endl;
  cout << "    + Exact scale in x5_encrypted : " << x5_encrypted.scale() << endl;
  cout << "    + Exact scale in x4_encrypted_coeff4: " << x4_encrypted_coeff4.scale() << endl;
  cout << "    + Exact scale in x3_encrypted: " << x3_encrypted.scale() << endl;
  cout << "    + Exact scale in x2_encrypted_coeff2: " << x2_encrypted_coeff2.scale() << endl;
  cout << "    + Exact scale in x1_encrypted: " << x1_encrypted.scale() << endl;
  cout.copyfmt(old_fmt1);

  cout << "Normalize scales to 2^30." << endl;
  x8_encrypted_coeff8.scale() = pow(2.0, 30);
  x7_encrypted_coeff7.scale() = pow(2.0, 30);
  ...
  evaluator.mod_switch_to_next_inplace(x6_encrypted_coeff6);
  evaluator.mod_switch_to_next_inplace(x5_encrypted);
  evaluator.mod_switch_to_next_inplace(x4_encrypted_coeff4);
  ...
  cout << "Compute -0.0007634 * x^8 -0.00001257 * x^7 + 0.01584 * x^6 + 0.00001584 * x^5 - 0.1188 * x^4 - 0.0005484 * x^3 + 0.4935 * x^2 + 0.5004 * x + 0.1009." << endl;
  Ciphertext encrypted_result=x8_encrypted_coeff8;
  evaluator.add_inplace(encrypted_result, x7_encrypted_coeff7);
  evaluator.add_inplace(encrypted_result, x6_encrypted_coeff6);
  ...
  cout << "    + Scale of encrypted_result before rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(encrypted_result);
  cout << "    + Scale of encrypted_result after rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  ...
  for(int i=0; i<slot_count ; i++){
    double temp = relu(input[i]); // function -0.0001734 * x^8 -0.00001257 * x^7 + 0.01584 * x^6 + 0.00001584 * x^5 - 0.1188 * x^4 - 0.0005484 * x^3 + 0.4935 * x^2 + 0.5004 * x + 0.1009
  ...
  cout<<"Result by PA ciphertest>>"<<endl;
