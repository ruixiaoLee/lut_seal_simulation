...
int main(int argc, char const *argv[]) {
  ...
  cout << "Evaluating polynomial - 0.01519 * x^4 - 0.0001013 * x^3+ 0.2734 * x^2 + 0.5004 * x + 0.1758 ..." << endl;
  double coeff4 = -0.01519;
  double coeff3 = -0.0001013;
  double coeff2 = 0.2734;
  double coeff1 = 0.5004;
  double coeff0 = 0.1758;
  vector<double> v_coeff4, v_coeff3, v_coeff2, v_coeff1, v_coeff0;
  Plaintext plain_coeff4, plain_coeff3, plain_coeff2, plain_coeff1, plain_coeff0;
  Ciphertext ct_coeff4, ct_coeff3, ct_coeff2, ct_coeff1, ct_coeff0;
  ...
  cout << "    + Exact scale in -0.01519 * x^4: " << x4_encrypted_coeff4.scale() << endl;
  cout << "    + Exact scale in -0.0001013 * x^3: " << x3_encrypted.scale() << endl;
  cout << "    + Exact scale in 0.2734 * x^2: " << x2_encrypted_coeff2.scale() << endl;
  cout << "    + Exact scale in 0.5004 * x: " << x1_encrypted.scale() << endl;
  ...
  cout << "Compute - 0.01519 * x^4 - 0.0001013 * x^3+ 0.2734 * x^2 + 0.5004 * x." << endl;
  Ciphertext encrypted_result=x4_encrypted_coeff4;
  ...
  cout << "    + Scale of - 0.01519 * x^4 - 0.0001013 * x^3+ 0.2734 * x^2 + 0.5004 * x before rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(encrypted_result);
  cout << "    + Scale of - 0.01519 * x^4 - 0.0001013 * x^3+ 0.2734 * x^2 + 0.5004 * x after rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  ...
  cout << fixed << setprecision(10);
  cout << "    + Exact scale in - 0.01519 * x^4 - 0.0001013 * x^3+ 0.2734 * x^2 + 0.5004 * x: " << encrypted_result.scale() << endl;
  cout << "    + Exact scale in 0.1758: " << ct_coeff0.scale() << endl;//- 0.01519 * x^4 - 0.0001013 * x^3+ 0.2734 * x^2 + 0.5004 * x + 0.1758
