...
int main(int argc, char const *argv[]) {
  ...
  /* make a random value vector */
  input=readTable("randomInput/test_normal.txt");
  input.resize(slot_count);

  cout << "Evaluating polynomial -0.00004065 * x^8 + 0.001255 * x^6 -0.01883 * x^4 -0.00001078 * x^3 + 0.2482 * x^2 + 0.5 * x + 0.0002781 ..." << endl;
  double coeff8 = -0.00004065;
  double coeff6 = 0.001255;
  double coeff4 = -0.01883;
  double coeff3 = -0.00001078;
  double coeff2 = 0.2482;
  double coeff1 = 0.5;
  double coeff0 = 0.0002781;
  vector<double> v_coeff8, v_coeff6, v_coeff5, v_coeff4, v_coeff3, v_coeff2, v_coeff1, v_coeff0;
  Plaintext plain_coeff8, plain_coeff6, plain_coeff5, plain_coeff4, plain_coeff3, plain_coeff2, plain_coeff1, plain_coeff0;
  Ciphertext ct_coeff8, ct_coeff6, ct_coeff5, ct_coeff4, ct_coeff3, ct_coeff2, ct_coeff1, ct_coeff0;
  ...
  cout<<"Compute -0.0001753 * x^8."<<endl;
  ...
  cout << "    + Scale of -4.065e-05 x^8 + 0.001255 x^6  - 0.01883 x^4- 1.078e-05 x^3 + 0.2482 x^2 + 0.5 x before rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(encrypted_result);
  cout << "    + Scale of -4.065e-05 x^8 + 0.001255 x^6  - 0.01883 x^4- 1.078e-05 x^3 + 0.2482 x^2 + 0.5 x after rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  ...
  cout << "    + Exact scale in ct_coeff0: " << ct_coeff0.scale() << endl;
  ...
  for(int i=0; i<slot_count ; i++){
    double temp = swish(input[i]); // function -4.065e-05 x^8 + 0.001255 x^6  - 0.01883 x^4- 1.078e-05 x^3 + 0.2482 x^2 + 0.5 x + 0.0002781
