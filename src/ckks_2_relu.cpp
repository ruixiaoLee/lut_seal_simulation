...
int main(int argc, char const *argv[]) {
  ...
  cout << "Evaluating polynomial 0.1563 * x^2 + 0.5003 * x + 0.2812 ..." << endl;
  double coeff2 = 0.1563;
  double coeff1 = 0.5003;
  double coeff0 = 0.2812;
  ...
  /* encrypt the coefficients and inputs*/
  ...
  cout << "Compute 0.5003 *x." << endl;
  evaluator.multiply_inplace(x_encrypted, ct_coeff1); //  x_encrypted => x * coeff1
  cout << "Compute 0.1563 * x^2." << endl;
  Ciphertext x2_encrypted_coeff2 = x2_encrypted; //  x2_encrypted_coeff2 => x^2 * coeff2
  ...
  cout << "    + Exact scale in 0.1563 * x^2: " << x2_encrypted_coeff2.scale() << endl;
  cout << "    + Exact scale in   0.5003 * x: " << x_encrypted.scale() << endl;
  ...
  cout << "Compute 0.1563 * x^2 + 0.5003 * x " << endl;
  Ciphertext encrypted_result;
  evaluator.add(x2_encrypted_coeff2, x_encrypted, encrypted_result);
  evaluator.relinearize_inplace(encrypted_result, relin_keys);
  cout << "    + Scale of 0.1563 * x^2 + 0.5003 * x before rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  evaluator.rescale_to_next_inplace(encrypted_result);
  cout << "    + Scale of 0.1563 * x^2 + 0.5003 * x rescale: " << log2(encrypted_result.scale()) << " bits" << endl;
  ...
  cout << fixed << setprecision(10);
  cout << "    + Exact scale in 0.1563 * x^2 + 0.5003 * x : " << encrypted_result.scale() << endl;
  cout << "    + Exact scale in                     0.2812: " << ct_coeff0.scale() << endl;
  ...
  for(int i=0; i<slot_count ; i++){
    double temp = relu(input[i]); // function 0.1563 * x^2 + 0.5003 * x + 0.2812
    true_result.push_back(temp);
  }
  cout<<"Real result by ptxt >>"<<endl;
  ...
  cout<<"Result by ctxt poly_approximation >>"<<endl;
